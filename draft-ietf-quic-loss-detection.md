...
transport.  The QUIC protocol is described in 
ietf-quic-transport-protocol.
...
# Design of the QUIC Transmission Machinery
   ...
   *  Retransmittable frames are frames requiring reliable delivery.
...
## Variables of interest
   ...
   *  alarm_mode: QUIC maintains a single loss detection alarm, which
      switches between various modes.  This mode is used to determine
      the duration of the alarm.

   *  handshake_count: The number of times the handshake packets have
   ...
   *  time_loss: When true, loss detection operates solely based on
...
## On Sending a Packet
   ...
   *  is_retransmittble: A boolean that indicates whether the packet
      contains at least one frame requiring reliable deliver.  The
      retransmittability of various QUIC frames is described in
      ietf-quic-transport-protocol.  If false, it is still acceptable for an
