...
area: Transport
workgroup: QUIC
keyword: [fec, bbr, next generation, delay-sensitive congestion control]
...
A Probe Timeout (PTO) the triggers of sending one or two probe datagrams when
...
dictated by the packet and time threshold mechanisms; see {{ack-loss-detection}}.
...
TCP {{?RFC5682}}. The timeout computation is based on TCP's RTO period {{?RFC6298}}.
...
restarting the connection process.  A Retry packet indicates that the Initial packet
...
pacing limited, the congestion window is under-utilized. This can happen due to
