...
loss of a packet containing handshake data the same as other losses.
...
When the Initial keys and Handshake keys are discarded ({{discarding-packets}}),
any Initial packets and Handshake packets can no longer be acknowledged, so
they are removed from bytes in flight. When Initial or Handshake keys are
...
encrypted with them are either acknowledged or declared lost. However, Initial
and Handshake secrets are discarded as soon as handshake and 1-RTT keys are
proven to be available to both client and server;
...
an initial value.  Endpoints SHOULD use an initial congestion window of ten times
the maximum datagram size (max_datagram_size), while limiting the window
to the larger of 14,720 bytes or twice the maximum datagram size. This follows
...
or when the ECN-CE count reported by its peer increases. A sender that is already in
a recovery period stays in it and does not reenter it.
