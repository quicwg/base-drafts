...
QUIC connections carry a connection ID, an opaque value assigned by the intended
...
verify addresses of endpoints that change network connectivity, while
...
: Octet 1 contains the lengths of the two connection ID fields that follow it.
  These lengths are encoded as two 4-bit unsigned integers. The length of the
  Destination Connection ID occupies the high bits of the octet and the length
  of the Source Connection ID occupying the low bits of the octet.  An encoded
  length of 0 indicates that the connection ID is also 0 octets in length.
  Non-zero encoded lengths are increased by 7 to get the full length of the
  connection ID, producing a length between 8 and 22 octets inclusive.  For
  example, an octet with the value 0xa0 describes a 17 octet Destination
  Connection ID and a zero octet Source Connection ID.
...
: The QUIC Version is a 32-bit field that follows the Source Connection
  ID.  This field indicates which version of QUIC is in use and determines how
  the rest of the protocol fields are interpreted.

Packet Number:

: The Packet Number is a 32-bit field that follows the Version.
...
packet it receives in the Destination Connection ID field; similarly, the value
of the Source Connection ID field is set to the Destination Connection ID from
the received packet.  The value for Source Connection ID is randomly selected by
a client and so gives clients some assurance that the server received the packet
...
This MUST NOT be zero length, but can be any other length.  If the client
received a Retry packet and is sending a second Initial packet, then it uses the
value from the Source Connection ID in the Retry packet.
...
Destination Connection ID that each endpoint uses.  Each endpoint sends the
value that they wish their peer to use in the Source Connection ID field.  Upon
receiving a packet, each endpoint sets the Destination Connection ID to match
the value of the Source Connection ID that they receive.
...
If the Destination Connection ID is zero length, but the packet matches the
...
packets with zero-length Destination Connection ID fields if they do not meet
...
An endpoint that receives a packet from a new address with either a previously
unused connection ID or an empty connection ID MUST use the next available
connection ID for any packets it sends to that address.  Failing to do this
would allow for use of that connection ID to link activity on new paths.  An
endpoint SHOULD use a new connection ID if it observes a change in address
...
- Made connection IDs symmetrical and variable length.  Removed
  omit_connection_id and the corresponding short header flag.
